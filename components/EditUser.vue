    ...
    BForm(@submit.prevent="toggleSubmitUser" autocomplete="off")
      ...
      b.d-flex.justify-content-around.align-items-center.w-100
        b-row.my-5.justify-content-around.align-items-center.w-50
          b-button.border-0(variant='outline-dark' @click="toggleCancel")
...
export default class EditUser extends Vue {
  ...
  private async getUserById () {
    const result = await this.$axios.$get('/user/user', { params: { id: this.userId } })
    const { data } = await result

    if (data.success) {
      const user = data.data
      this.$data.formUser.token = user.token
      this.$data.formUser.email = user.email
      this.$data.formUser.firstname = user.firstname
      this.$data.formUser.lastname = user.lastname
      this.$data.formUser.secondLastname = user.secondLastname
      this.$data.formUser.username = user.username
      this.$data.formUser.curp = user.curp
      this.$data.formUser.rfc = user.rfc
      this.$data.formUser.birthdate = formatDate(user.birthdate)
      this.$data.formUser.primaryPhoneNumber = user.primaryPhoneNumber
      this.$data.formUser.secondaryPhoneNumber = user.secondaryPhoneNumber
      this.$data.formUser.biometricsFlag = Number(user.biometricsFlag)
      this.$data.formUser.biometricsType = user.biometricsType
      this.$data.formUser.biometricFile = user.biometricFile
      this.$data.formUser.avatarImage = user.avatarImage
      this.$data.formUser.role = user.role
      this.$data.formUser.securityLogFlag = Number(user.securityLogFlag)
      this.$data.formUser.privateSecurityKey = user.privateSecurityKey
      this.$data.formUser.vigilantModeFlag = Number(user.vigilantModeFlag)
      this.$data.formUser.mobileLoginFlag = Number(user.mobileLoginFlag)
      this.$data.formUser.twoFactorAuthFlag = Number(user.twoFactorAuthFlag)
      this.$data.formUser.twoFactorAuthRecord = user.twoFactorAuthRecord
    }
  }
  ...
  async toggleSubmitUser () {
    try {
      ...
      if (!this.$v.$invalid) {
        ...
        const { data } = await this.$axios.$put('/user/user', frm, { headers: { 'Content-Type': 'multipart/form-data' } })

        if (data.success) {
          this.makeToast('success', 'EDICIÓN DE USUARIO', '¡El usuario se actualizó exitosamente!')
        } else {
          this.makeToast('warning', 'EDICIÓN DE USUARIO', data.msg)
        }
      }
    } catch (error) {
      this.makeToast('danger', 'EDICIÓN DE USUARIO', '¡Intenta nuevamente!')
    }
  }
