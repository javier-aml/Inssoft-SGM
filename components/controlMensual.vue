...
b-container.text-center(fluid)
      ...
      b-container
        b-row
          b-col.input-group.col-8.float-end(style='margin-left: 24%;')
            ...
            input#ano.form-control(style='margin-right : 2%;' type='text' placeholder='YY/YYYY' aria-describedby='inputGroupPrepend').input-group.col-4.float-end
  ...
  .row
    ...
    .col
      a#jsonDiario.col-2.float-end.button.btn.btn-ekide.d-none.d-lg-block(@click="DownloadJson" type='button' style='margin-right: 13%;') Generar json Mensual
      br
...
export default class controlMensual extends Vue {
  $axiosSgm: any
  async mounted () {
    const fecha = '2023-01'
    const SatData:any = await this.$axiosSgm.$post(`/mensual-natgas/${fecha}`)
    Fecha(fecha)
  ...
  async changeFecha () {
    ...
    const fecha = `${ano.value}-${mes.value}`
    const SatData:any = await this.$axiosSgm.$post(`/mensual-natgas/${fecha}`)
    Fecha(fecha)
    const CompraData:any = SatData.tabla
    const VentaData:any = SatData.tablaVenta
  ...
  DownloadJson () {
    ...
    const ano:any = document.querySelector('#ano')
    const fecha = `${ano.value}-${mes.value}`
    window.open(`${process.env.SgmUrlBase}/Mensual/Download/ZipMensual/${fecha}`)
  }
  ...
  DownloadExcel () {
    ...
    const ano:any = document.querySelector('#ano')
    const fecha = `${ano.value}-${mes.value}`
    window.open(`${process.env.SgmUrlBase}/Mensual/Download/ExcelMensual/${fecha}`)
  }
}
