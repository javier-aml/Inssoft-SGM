...
$.post("/almacen/lector/QR",{result}, function( data ) {
  ...
  if (data == true) {       
    ...
    text += `<li class="list-group-item"><a>${data.length}</a></li>`
    $("#listDPedido").empty()
    $("#listDPedido").append(text);
    $("#TotalDiferentes").empty()
    $("#TotalDiferentes").append(`<p><strong>Total: </strong>${data.length}</p>`);
...
$("#addUbicacion").on("click",function(){
  ...
  $.post("/almacen/add/ubicacion",{nQR: obj.nQR,ubicacion:ubicacion,nParte:obj.nParte,Cantidad:obj.cantidad,dPedido:obj.dPedido }, function( data ) {
